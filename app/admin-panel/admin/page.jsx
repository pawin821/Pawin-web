            ...
            <div className="p-6">
              ...
              <div className="mt-6 flex justify-end space-x-3">
                {currentDocument?.status !== 'approved' && currentDocument?.status !== 'rejected' && (
                  <>
                    <button onClick={async ()=>{await updateBadge(userId,""); console.log(userId); alert("Badge will be updated in 10 secs"); window.location.reload()}}
                      className="px-4 py-2 border border-red-300 rounded-md text-red-700 bg-white hover:bg-red-50 flex items-center"
                    >
                      <XCircle className="h-4 w-4 mr-2" />
                      Reject Document
                    </button>
                    <button onClick={async ()=>{ await updateBadge(userId,"1"); console.log(userId); alert("Badge will be updated in 10 secs"); window.location.reload()}}
